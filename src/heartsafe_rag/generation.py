...
class GenerationService:
    ...
    def route_query(self, query: str, callbacks: list[Any] | None = None) -> str:
        """Determine whether the query is HF-related or general."""
        classification = self.router_chain.invoke({"question": query}, config={"callbacks": callbacks}).strip()
    ...
    def generate_response(
        ...
        Args:
            ...
            category: Optional category if already determined.
        """
        ...
        if category is None:
            category = self.route_query(query, callbacks=callbacks)

        if category == "HF_RELATED":
            if not context_docs:
                return "This appears to be a heart failure–related question, but no clinical documents were retrieved."

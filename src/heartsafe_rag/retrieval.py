...
class HybridRetriever:
    ...
    def get_retriever(self, weights: list[float] = [0.4, 0.6]) -> BaseRetriever:
        ...
        try:
            ...
            self._ensemble_retriever = EnsembleRetriever(retrievers=[bm25_retriever, faiss_retriever], weights=weights)
...
class RetrievalService:
    ...
    def _initialize_retriever(self) -> BaseRetriever:
        ...
        try:
            ...
            logger.error(f"Failed to initialize RetrievalService: {e!s}")
            raise e

    def retrieve(self, query: str, callbacks: list[Any] | None = None) -> list[Document]:
        """

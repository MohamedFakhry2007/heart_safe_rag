...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        # 1. Route Query
        category = gen_service.route_query(query, callbacks=callbacks)
        ...
        if category == "HF_RELATED":
            ...
            context_docs = ret_service.retrieve(query, callbacks=callbacks)

        # 3. Generate Response
        # We pass docs (empty or populated) to the generator
        answer = gen_service.generate_response(query, context_docs, callbacks=callbacks, category=category)
        ...
        return ChatResponse(answer=answer, category=category, sources=sources_response)
